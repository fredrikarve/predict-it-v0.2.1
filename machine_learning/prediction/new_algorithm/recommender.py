...
import numpy as np
# from recommendermap import dfFunctions
from machine_learning.prediction.new_algorithm.dfFunctions import ItemFinder, \
    BatchGenerator
from machine_learning.prediction.new_algorithm import tf_models


class SVDmodel(object):
    '''
    Class to create SVD models and NSVD models. This class does not deal with
    ...
    def __init__(self,
        ...
        if model == 'nsvd':
            self.finder = ItemFinder(df, self.users, self.items,
                                     self.ratings, nsvd_size)

    def data_separation(self):
        '''
        Method that randomizes the dataframe df and separate it
        in tree parts: 80% in training, 10% in test and 10% in validation.
    ...
    def training(self,
        ...
        '''
        self.train_batches = BatchGenerator(self.train,
                                            batch_size,
                                            self.users,
                                            self.items,
                                            self.ratings)

        self.test_batches = BatchGenerator(self.test,
                                           batch_size,
                                           self.users,
                                           self.items,
                                           self.ratings)

        self.valid_batches = BatchGenerator(self.valid,
                                            len(self.valid),
                                            self.users,
                                            self.items,
                                            self.ratings)
