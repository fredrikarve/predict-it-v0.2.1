...
from machine_learning.prediction.new_algorithm.utils import rmse, \
    status_printer
import time
import os
...
def inference_nsvd(user_batch,
    ...
    with tf.name_scope('Declaring_variables'):
        ...
        w_item2 = tf.get_variable(name='w_item2',
                                  ...
                                  initializer=initializer)
        zero = np.zeros(dim)
        fake_item = tf.constant(np.array([zero]), dtype=tf.float32,
                                shape=[1, dim], name='fake')
...
class SVD(object):
    ...
    data information from the class recommendermap.SVDmodel and sets the
    ...
    def set_graph(self,
        ...
        with self.graph.as_default():
            ...
            filename = os.path.join(
                root_dir,
                'machine_learning/prediction/new_algorithm/checkpoints/')
            save_dir = os.path.abspath(os.path.realpath(filename))
            ...
            with tf.name_scope('accuracy'):
                difference = tf.pow(tf.subtract(
                    self.infer, self.tf_rate_batch), 2)
    ...
    def training(self,
        ...
        # init_op = tf.global_variables_initializer()  # Added by Carl Eriksson
        with tf.Session(graph=self.graph) as sess:
            tf.initialize_all_variables().run()
            # init_op.run()   # Added by Carl Eriksson

            # A warning tells us to change to sess.run(init_op) but as long as this works, let's keep it this way
            if verbose:
                print('{} {} {} {}'.format('step',
                                           'batch_error',
                                           'test_error',
