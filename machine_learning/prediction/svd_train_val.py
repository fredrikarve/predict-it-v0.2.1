...
# Changes have been made so that the ratings.dat file serves as a training-set
# and the requests.dat- file serves as a test set. Currently the requests.dat
# can only contain one row, if more rows are inputted into requests.dat only
# the last one will be returned. The User-ID and item-ID gets it's ID reduced
# by one (if id = 3699, the output will be id = 3698.0). This is probably
# caused by line 10 and 11 in file dataio.py.
...
def get_data():
    ...
    filename = os.path.join(
        root_dir,
        'machine_learning/prediction/new_algorithm/ml-1m/requests.dat')
    filename = os.path.abspath(os.path.realpath(filename))
    ...
    df_train = df[0:split_index]
    df_test = df[split_index:].reset_index(drop=True)
    return df_train, df2    # OBS, I'm not using df_test, instead I'm using
                            # df2, imported from ratings2.dat. df_test might be
                            # removed from the code when we know better how it
                            # works. All of the 'split_index' related stuff
                            # might also get removed, not sure yet. -Calle
...
def svd(train, test):
    ...
    infer, regularizer, predict = ops.inference_svd(user_batch, item_batch,
                                                    user_num=USER_NUM,
                                                    item_num=ITEM_NUM, dim=DIM,
                                                    device=DEVICE)
    global_step = tf.contrib.framework.get_or_create_global_step()
    _, train_op = ops.optimization(infer, regularizer, rate_batch,
                                   learning_rate=0.001, reg=0.05,
                                   device=DEVICE)

    init_op = tf.global_variables_initializer()
    with tf.Session() as sess:
        sess.run(init_op)
        filename = os.path.join(root_dir, 'machine_learning/prediction/ml-1m')
        filename = os.path.abspath(os.path.realpath(filename))
        summary_writer = tf.summary.FileWriter(logdir=filename,
                                               graph=sess.graph)
        ...
        for i in range(EPOCH_MAX * samples_per_batch):
            ...
            if i % samples_per_batch == 0:
                ...
                train_err_summary = make_scalar_summary('training_error',
                                                        train_err)
...
# This function takes in the id of a User and an item. The function will return
# a prediction of the users Rating on the item
def get_rating(user_id, item_id):
    request_data = str(user_id) + '::' + str(item_id) + '::5::978298459'
    col_names = ['User', 'item', 'rate', 'st']
    request_data = pd.read_fwf(StringIO(request_data), delimiter='::',
